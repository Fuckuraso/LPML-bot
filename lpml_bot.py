...
async def Alert():
    ...
    while not client.is_closed():
        channel = client.get_channel(751725397582020668)
        f = open('date.txt', 'r')
        lst=[]
        for s in f:
            s = s.rstrip()
        ...
        k=len(lst)
        for i in range(0, k):
            if str(time.strftime("%x"))== str(lst[i]):
                o=i+1
                await channel.send(f'Сьогодні потрібно здати ДЗ№{o}')  
...
@client.command()
async def hadd(ctx, a1, a2, *, a3):#команда яка чистить список відміток і список присутніх
    global hm
    global f
    global s
    dt=[]
    ...
    number=str(a1)
    date=str(a2)
    if str(ctx.author.id)==str(428159463002734595):
        dt.append(date)
        hm.append(f'ДЗ№{number}: %s. Дата здачі: {date}' %text )
        print(hm)
        m= open('date.txt', 'wt')  
        for k in dt:
            m.write(k+'\n')
        ...
        for s in hm:
       	    f.write(s+'\n')
        f.close()
        await ctx.send('Д/з додано.')
    else: 
        await ctx.send('Ви не можете додати ДЗ!!!!')
@client.command()
async def hp(ctx, a1):
    global f
    global s
